...
public class VideoObjectTrackingAnnotationProcessor implements MLApiResponseProcessor {

  private static final long serialVersionUID = 1L;

  private static final Logger LOG =
      LoggerFactory.getLogger(VideoObjectTrackingAnnotationProcessor.class);
  ...
  public VideoObjectTrackingAnnotationProcessor(
      String tableId,
  ...
  private static class SchemaProducer implements TableSchemaProducer {
    ...
    public TableSchema getTableSchema() {
      ...
      fields.add(new TableFieldSchema().setName(Field.CONFIDENCE).setType("FLOAT"));
      fields.add(
          new TableFieldSchema()
  ...
  public ProcessorResult process(GCSFileInfo fileInfo, GeneratedMessageV3 r) {
    ...
    counter.inc(annotationResults.getObjectAnnotationsCount());
    ProcessorResult result =
        new ProcessorResult(ProcessorResult.VIDEO_OBJECT_TRACKING, destination);
    for (ObjectTrackingAnnotation annotation : annotationResults.getObjectAnnotationsList()) {
      ...
      if (relevantEntities != null
          ...
          && annotation.getConfidence() >= confidenceThreshold) {
  ...
  public TableDetails destinationTableDetails() {
    return TableDetails.create(
        "Google Video Intelligence API object tracking annotations",
